...
public class DragMenu : MonoBehaviour, IDragHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    private bool isAfterHalfPoint() {
        ...
        {
            isOpen = false;
            return sideMenuRectTransform.anchoredPosition.y < (height - (height / 16));
        }
    }
    ...
    public void closeMenu()
    {
        if (isOpen)
        {
            StartCoroutine(HandleMenuSlide(0.37f, sideMenuRectTransform.anchoredPosition.y, GetMinPosition()));
            isOpen = false;
        }
    }
